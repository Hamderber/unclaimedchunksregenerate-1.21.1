...
public class Config {
    ...
    public static final int MAX_CLAIM_DISTANCE = 10;
    ...
    static {
        ...
        ConfigAPI.CHUNK_SCAN_FLAGGING_CHANCE_MODULO = BUILDER
            .comment(" Factor impacting how likely a chunk is to be flagged for a scan of block changes. " +
                    "Bigger number is less scanning.")
            .defineInRange("chunkScanFlaggingModulo", 7, 1, 512); // default 7
        
        ConfigAPI.SKIP_CHUNK_SCAN_BELOW_TPS = BUILDER
            .comment(" Restricts the chunk scanning to be limited by server TPS. If the TPS is below this value, " +
                    "the chunk scan won't happen.")
        ...
        registerDimension("minecraft:overworld", 365, 3, 25, 0.5,
                true, true, false, true);
        registerDimension("minecraft:the_nether", 30, 1, 50, 0.8,
                true, true, false, true);
        registerDimension("minecraft:the_end", 30, 0, 10, 1.0,
                true, true, false, true);
        registerDimension("allthemodium:mining", 7, 50, 0, 0.3,
                true, true, false, true);
        registerDimension("allthemodium:the_other", 30, 1, 50, 0.8,
                true, true, false, true);
        registerDimension("allthemodium:the_beyond", 365, 5, 5, 1.0,
                true, true, false, true);
        registerDimension("lostcities:lostcity", 180, 2, 25, 0.5,
                true, true, false, true);
        registerDimension("twilightforest:twilight_forest", 180, 3, 25, 0.5,
                true, true, false, true);
        registerDimension("aether:the_aether", 30, 3, 25, 1.0,
                true, true, false, true);
        registerDimension("the_bumblezone:the_bumblezone", 30, 2, 25, 0.5,
                true, true, false, true);
    ...
    private static void registerDimension(String id,
    	...
    	IntValue regenPeriod = BUILDER
	        .comment(" How many tick-based game days before unclaimed chunks in " + id +
                " are eligible for regeneration",
    	...
    	IntValue distance = BUILDER
    	        .comment(" The cubic radius around a claimed chunk in " + id +
                    " where unclaimed chunks won't regenerate.")
    	        .defineInRange(id + ".distanceFromClaim", distanceFromClaim, 0, MAX_CLAIM_DISTANCE);
    	CLAIM_DISTANCE.put(id, distance);
	    

	     IntValue airDeltaSetting = BUILDER
            .comment(" The minimum +/- change in estimated air blocks for " + id +
                " required for a chunk to be elligble for regeneration.",
         		" Defaults are based on potential chunk changes during generation, such as water flowing from a " +
                "spawned source or vertical plants growing. Less than one (< 1) to disable this feature.")
	     ...
	     DoubleValue percentOfChunkToScan = BUILDER
                 .comment(" The lower percent of the chunk to scan for changes. Note that most of a chunk is just  " +
                     "air on top, so this value may be deceptively low.")
