...
Chunks lacking FTB claims regenerate in configurable dimensions and periodicities. The Overworld is defaulted to 365 Minecraft days and the End and Nether are at 30. When a chunk's age has surpassed the configured threshold, it is flagged for regeneration. The next time that a flagged chunk is loaded, it is loaded as if it were just created. This means that structures, ores, loot chests, original entities, etc. will all be respawned. The underlying library mod supports randomized trees, ores, and animals during chunk regeneration. Those features are all configured through this mod.
 
KEY FEATURES:
Chunks are reloaded as if brand new once they reach the configured age.
Configurable range around FTB claimed chunks to skip the regeneration process. Chunks actually claimed by FTB Chunks will never regenerate.
